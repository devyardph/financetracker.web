...
namespace DYS.FinanceTracker.Shared.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        public static string EncodeBase64(this byte[] content)
        {
        ...
        public static byte[]? DecodeBase64(this string encodedText)
        {
            if (encodedText == null) return default;
        ...
        {
            var invalidRegStr = "";
            if (string.IsNullOrEmpty(invalidRegStr))
            {
        ...
        public static bool IsGuidEmpty(this string input)
        {
            return string.Equals(input, "00000000-0000-0000-0000-000000000000");
        }

        public static bool ToBoolean(this string input)
        {
            if (Boolean.TryParse(input, out bool output))
                return output;
            else
                return false;
        }
        ...
        public static string Truncate(this string value, int maxLength)
        {
            ...
            var suffix = value.Length > maxLength ? "..." : string.Empty;
            return string.IsNullOrEmpty(value) ? value : value.Substring(0, Math.Min(value.Length, maxLength)) + suffix;
        }

        public static bool IsValidEmail(this string input)
        {
            var pattern = @"^\s*[\w\-\+_']+(\.[\w\-\+_']+)*\@[A-Za-z0-9]([\w\.-]*[A-Za-z0-9])?\.[A-Za-z][A-Za-z\.]*[A-Za-z]$";
            var email = !string.IsNullOrEmpty(input) ? input.Trim() : string.Empty;
            var valid = !string.IsNullOrWhiteSpace(email) &&
              new Regex(pattern).IsMatch(email);
            return valid;
        }

        public static bool IsValidNZPhone(this string input)
        {
            var phone = !string.IsNullOrEmpty(input) ? input.Trim() : string.Empty;
            return !string.IsNullOrWhiteSpace(phone) &&
               new Regex(@"^(?:\+?(64))? ?(?:\((?=.*\)))?(0?[2-57-8])\)? ?(\d\d(?:[- ](?=\d{3})|(?!\d\d[- ]?\d[- ]))\d\d[- ]?\d[- ]?\d{3})$").IsMatch(phone);

        }

        public static bool Has(this string input, string compare)
        {
            var value = !string.IsNullOrEmpty(input) ? input.Trim() : string.Empty;
            return !string.IsNullOrWhiteSpace(value) &&
               value.Contains(compare, StringComparison.OrdinalIgnoreCase);
        }
        ...
        public static string GetQueryParameter(this string url, string parameter)
        {
        ...
        public static string YesOrNo(this bool? YesOrNo)
        {
            if (YesOrNo == null) return string.Empty;
        ...
        public static string GenerateAlphaNumericCode(int max = 8)
        {
            var chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
            var random = new Random();
            var list = Enumerable.Repeat(0, max).Select(x => chars[random.Next(chars.Length)]);
            return string.Join("", list);
        }

        public static string GenerateToken(int max = 8)
        {
        ...
        public static string GetSubString(this string? text, int max = 2)
        {
        ...
        public static List<string> SplitString(this string input)
        {
            ...
            return output.Split(new Char[] { ',' }).Select(query => query.Trim()).ToList();
        }
        ...
        public static string ToCamelCase(this string title) => !string.IsNullOrEmpty(title) ? CultureInfo.CurrentCulture.TextInfo.ToTitleCase(title.ToLower()) : "";
        public static string CapitalizeWords(this string input)
        {
            ...
            {
                words[i] = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(words[i].Trim());
            }

            return string.Join(", ", words);
        }

        public static List<string> TextStartsWith(this string input, string character = "@")
        {
            if (string.IsNullOrEmpty(input)) return new List<string>();

            var words = input.Split(" ").ToList();
            return words?.Where(query => query.StartsWith(character)).ToList() ?? new List<string>();
        }
        public static string To12HourFormat(this TimeSpan timeSpan)
        {
            DateTime baseDateTime = DateTime.Today; // Use today's date as a reference
            DateTime dateTimeWithTimeSpan = baseDateTime.Add(timeSpan);
            return dateTimeWithTimeSpan.ToString("hh:mm tt");
        }
        public static bool NotEmpty(this string input) => !string.IsNullOrEmpty(input);
        ...
        public static string GetInitials(this string input)
        {
            ...
            var initials = words.Take(2)
                                .Select(word => word.Length > 0 ? word.Substring(0, 1) : string.Empty);

            var result = string.Join("", initials);

            return result.ToUpper();
        }
        public static string CapitalizeFirstLetter(this string input) => string.IsNullOrEmpty(input) ? string.Empty : char.ToUpper(input[0]) + input.Substring(1);
        public static string Capitalize(this string input, bool onlyFirstLetter = false)
        {
            if (string.IsNullOrEmpty(input)) return string.Empty;
            string[] words = input.Split(' ');
            for (int i = 0; i < (onlyFirstLetter ? 1 : words.Length); i++)
            {
        ...
        public static string TrimGuid(this Guid input)
        {
            return input.ToString().Replace("-", string.Empty);
        }

        public static string GenerateCode(string prefix, int id = 0) => $"{prefix}-{id.ToString("D5")}";
        public static string GenerateCode(this Guid Id) => $"{Id.ToString("N").Substring(0, 12).ToUpper()}";
        ...
        public static string DisplayText(this string input, string placeholder)
        {
            var text = input?.Trim();
